...
local captured_windows ={
    g3x_left = {name="G3X Left Display"},
    g3x_right = {name="G3X Right Display"},
}
...
function module.start(config, aircraft)
    ...
    for key, def in pairs(captured_windows) do
        def.object = mapper.view_elements.captured_window{name=def.name}
    end
    ...
    for viewix, view in ipairs(views) do
        ...
        local function change_active_component(cid)

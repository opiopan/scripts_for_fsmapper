...
local buttons = {
    warning = {x=70.012, y=49.563, attr=attr_safety_button, action=msfs.mfwasm.rpn_executer("(>K:MASTER_WARNING_ACKNOWLEDGE)")},
    caution = {x=394.863, y=49.563, attr=attr_safety_button, action=msfs.mfwasm.rpn_executer("(>K:MASTER_CAUTION_ACKNOWLEDGE)")},
    std = {x=839.326, y=88.781, attr=attr_std_button, action=msfs.mfwasm.rpn_executer("1 (>H:AS3000_PFD_1_BARO_PUSH)")},
    fd = {x= 1136.527, y=26.934, attr=attr_ap_button, action=msfs.mfwasm.rpn_executer("1 (>K:TOGGLE_FLIGHT_DIRECTOR)")},
    bank = {x= 1136.527, y=161.713, attr=attr_ap_button, action=msfs.mfwasm.rpn_executer("(>K:AP_MAX_BANK_INC)")},
    xfr = {x=1545.024, y=26.934, attr=attr_ap_button, action=msfs.mfwasm.rpn_executer("(L:XMLVAR_PushXFR) ! (>L:XMLVAR_PushXFR)")},
    ap = {x=1435.434, y=161.713, attr=attr_ap_button, action=msfs.mfwasm.rpn_executer("(A:AUTOPILOT DISENGAGED, Bool) ! if{ (>K:AP_MASTER) (A:AUTOPILOT MASTER, Bool) ! if{ (>H:Generic_Autopilot_Manual_Off) } els{ (A:AUTOPILOT YAW DAMPER, Bool) ! if{ (>K:YAW_DAMPER_TOGGLE) } } }")},
    yd = {x=1645.566, y=161.713, attr=attr_ap_button, action=msfs.mfwasm.rpn_executer("(>K:YAW_DAMPER_TOGGLE)")},
    lvl = {x=1953.606, y=81.671, attr=attr_lvl_button, action=msfs.mfwasm.rpn_executer("(>K:AP_WING_LEVELER) (A:AUTOPILOT WING LEVELER, Bool) if{ (A:AUTOPILOT MASTER, Bool) (>O:APStateWhenLevelerEnabled) (A:AUTOPILOT MASTER, Bool) ! if{ (>K:AUTOPILOT_ON) } (>K:AP_PITCH_LEVELER_ON) } els{ (>K:AP_PITCH_LEVELER_OFF) (O:APStateWhenLevelerEnabled) if{ (>K:AUTOPILOT_ON) } els{ (>K:AUTOPILOT_OFF) } }")},
}

...
do -- Airecraft which installs GMC 307 control panel
    ...
    panel_def.buttons = {
        ap = {x=921.168, y=32.867, attr=attr_button, image=assets.ap, action=msfs.mfwasm.rpn_executer("(A:AUTOPILOT DISENGAGED, Bool) ! if{ (>K:AP_MASTER) (A:AUTOPILOT MASTER, Bool) ! if{ (>H:Generic_Autopilot_Manual_Off) } els{ (A:AUTOPILOT FLIGHT DIRECTOR ACTIVE, Bool) ! if{ 1 (>K:TOGGLE_FLIGHT_DIRECTOR) } } }")},
        lvl = {x=1180.832, y=32.867, attr=attr_button, image=assets.lvl, action=msfs.mfwasm.rpn_executer("(>K:AP_WING_LEVELER) (A:AUTOPILOT WING LEVELER, Bool) if{ (A:AUTOPILOT MASTER, Bool) (>O:APStateWhenLevelerEnabled) (A:AUTOPILOT MASTER, Bool) ! if{ (>K:AUTOPILOT_ON) } (>K:AP_PITCH_LEVELER_ON) } els{ (>K:AP_PITCH_LEVELER_OFF) (O:APStateWhenLevelerEnabled) if{ (>K:AUTOPILOT_ON) } els{ (>K:AUTOPILOT_OFF) } }")},
    }
...
do -- King Air 350i
    ...
    panel_def.buttons = {
        warning = {x=85.694, y=48.744, attr=attr_caution, action=msfs.mfwasm.rpn_executer("1 (>H:Generic_Master_Warning_Push, Number)")},
        caution = {x=368.328, y=48.744, attr=attr_caution, action=msfs.mfwasm.rpn_executer("1 (>H:Generic_Master_Caution_Push, Number)")},
        half_bank = {x=1483.569, y=103.14, attr=attr_button, action=msfs.mfwasm.rpn_executer("(A:AUTOPILOT MAX BANK, degrees) 16 < if{ (>K:AP_MAX_BANK_INC) } els{ (>K:AP_MAX_BANK_DEC) }")},
        yd = {x=1802.731, y=103.14, attr=attr_button, action=msfs.mfwasm.rpn_executer("(>K:YAW_DAMPER_TOGGLE)")},
        ap = {x=1998.256, y=103.14, attr=attr_button, action=msfs.mfwasm.rpn_executer("(A:AUTOPILOT DISENGAGED, Bool) ! if{ (>K:AP_MASTER) (A:AUTOPILOT MASTER, Bool) ! if{ (>H:Generic_Autopilot_Manual_Off) } els{ (A:AUTOPILOT FLIGHT DIRECTOR ACTIVE, Bool) ! if{ 1 (>K:TOGGLE_FLIGHT_DIRECTOR) } } }")},
    }
...
aircraft_defs["Beechcraft King Air"] = {views={"G3X Touch PFD","G3X Touch MFD"}, aptype=2}
...
function context.start(config, aircraft)
    ...
    if aircraft_def.aptype == 1 then
        ...
        viewport:set_mappings{
            ...
            {event=g1000.EC2X.decrement, action=msfs.mfwasm.rpn_executer("100 (>K:AP_ALT_VAR_DEC)")},
            {event=g1000.EC2Y.increment, action=msfs.mfwasm.rpn_executer("1000 (>K:AP_ALT_VAR_INC)")},
            {event=g1000.EC2Y.decrement, action=msfs.mfwasm.rpn_executer("1000 (>K:AP_ALT_VAR_DEC)")},
    ...
    elseif aircraft_def.aptype == 2 then
        viewport:set_mappings{
            ...
            {event=g1000.EC8.increment, action=msfs.mfwasm.rpn_executer("(>K:VOR2_OBI_INC)")},
            {event=g1000.EC8.decrement, action=msfs.mfwasm.rpn_executer("(>K:VOR2_OBI_DEC)")},
            {event=g1000.EC8P.down, action=msfs.mfwasm.rpn_executer("(A:HEADING INDICATOR,degrees) (>K:VOR2_SET)")},
            {event=g1000.EC7Y.increment, action=msfs.mfwasm.rpn_executer("1 (>K:KOHLSMAN_INC) (>H:AP_BARO_Up)")},
            {event=g1000.EC7Y.decrement, action=msfs.mfwasm.rpn_executer("1 (>K:KOHLSMAN_DEC) (>H:AP_BARO_Down)")},
        }
    else
